...
/><ApiField
  ...
  required="false"
  description="FailurePolicy controls behaviour when the filter service is<br />unreachable, returns a non-2xx status, returns a malformed response,<br />or exceeds TimeoutSeconds.<br />- `PassThrough`: the original JSON-RPC message is used and the tool<br />  call continues. This is fail-open and is the default.<br />- `Fail`:        the gateway returns a JSON-RPC error to the client.<br />  Use this for evaluation workloads where an unscanned response must<br />  never be served.<br />Defaults to `PassThrough`."
/><ApiField
  name="forwardHeaders"
  type="string array"
  required="false"
  description="ForwardHeaders lists HTTP header names to copy from the client's<br />incoming request into the filter invocation. Header names are<br />case-insensitive. This enables tenant- or context-aware policy (for<br />example, forwarding an evaluation-run ticket ID or a tenant ID).<br />Authorization and other sensitive headers are NOT automatically<br />forwarded; if the filter needs them, list them explicitly."
/><ApiField
  name="mode"
  type="[MCPContentFilterMode](#mcpcontentfiltermode)"
  required="false"
  description="Mode selects between Enforce (default) and Shadow. In Shadow mode<br />the gateway still invokes the filter service (or in-process<br />dispatcher), records the would-be verdict via<br />X-Content-Filter-Status and mcp_filter_decisions_total, and emits<br />redaction audit events, but always forwards the ORIGINAL body to<br />the client (and backend on Request scope). Use Shadow during<br />pre-production rollout to measure false-positive and<br />false-negative rates on real traffic without impacting users.<br />Operators flip back to Enforce to activate actual enforcement<br />without a gateway restart.<br />Defaults to `Enforce`."
/><ApiField
  name="enabled"
  type="boolean"
  required="false"
