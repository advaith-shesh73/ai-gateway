...
{{/*
Fully qualified name for the content-filter deployment/service/etc. Falls back
to `<release>-content-filter` when no override is supplied via values.
*/}}
{{- define "ai-gateway-helm.contentFilter.fullname" -}}
{{- if .Values.contentFilter.fullnameOverride -}}
{{- .Values.contentFilter.fullnameOverride | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-content-filter" .Release.Name | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end -}}

{{/*
Selector labels for the content-filter workload. Keeping these distinct from
the controller's labels so both can coexist in the same release without Pod
selector overlap.
*/}}
{{- define "ai-gateway-helm.contentFilter.selectorLabels" -}}
app.kubernetes.io/name: {{ include "ai-gateway-helm.contentFilter.fullname" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/component: content-filter
{{- end -}}

{{/*
Common labels applied to every content-filter resource (Deployment, Service,
ConfigMap, ServiceAccount). Inherits the chart/version tags like the
controller so release tooling can locate these resources uniformly.
*/}}
{{- define "ai-gateway-helm.contentFilter.labels" -}}
helm.sh/chart: {{ include "ai-gateway-helm.chart" . }}
{{ include "ai-gateway-helm.contentFilter.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
...
{{/*
ServiceAccount name for the content-filter deployment. When creation is
disabled we honour the name override or fall back to "default".
*/}}
{{- define "ai-gateway-helm.contentFilter.serviceAccountName" -}}
{{- if .Values.contentFilter.serviceAccount.create -}}
{{- default (include "ai-gateway-helm.contentFilter.fullname" .) .Values.contentFilter.serviceAccount.name -}}
{{- else -}}
{{- default "default" .Values.contentFilter.serviceAccount.name -}}
{{- end -}}
{{- end -}}

{{/*
Returns imagePullSecrets for the content-filter deployment.
Component-specific values win; `global.imagePullSecrets` is the fallback.
*/}}
{{- define "ai-gateway-helm.contentFilter.imagePullSecrets" -}}
{{- $src := default .Values.global.imagePullSecrets .Values.contentFilter.imagePullSecrets -}}
{{- if $src -}}
{{- toYaml $src -}}
{{- end -}}
{{- end -}}
